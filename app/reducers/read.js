...
function loadMore(state, action) {
    // console.warn('>>>>>>>>>'+JSON.stringify(state)+JSON.stringify(action));
    try {
        state.articleList[action.typeId].events = state.articleList[action.typeId].events.concat(
            action.articleList.events
        );
    }catch(err){
